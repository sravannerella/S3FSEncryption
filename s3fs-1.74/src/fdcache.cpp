...
string Cryption::read(int fd){
  ...
  string FileContents(reinterpret_cast<char*>(fcontents), flength);

  return FileContents;
}

string Cryption::getKey(){
  string KeyFile = "/home/sravan/key.txt";
  string key;

  ifstream myfile;
  myfile.open(KeyFile.c_str());
  getline(myfile, key);
  myfile.close();
  return key;
}